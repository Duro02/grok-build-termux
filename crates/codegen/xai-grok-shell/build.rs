...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // There is no official Android ripgrep release asset. A host `rg` binary
    // must never be embedded into an Android executable, so use Termux's
    // package-managed `rg` at runtime unless the caller explicitly supplies
    // an Android-native binary.
    if target_os == "android" && path_override.is_none() {
        println!(
            "cargo:warning=Skipping bundled ripgrep for Android; install ripgrep in Termux or set GROK_SHELL_BUNDLE_RG_PATH to an Android-native rg"
        );
        return Ok(());
    }
